...
async function recipeView(recipe_id) {
    let response = await fetch(
        "http://localhost:5000/recipe/view?" +
...
function RecipeView(props) {
    ...
    async function processId() {
        ...
        let response = await recipeView(id_).catch((e) => {});

        if (response != null) {
            let photosP = [];
            for (let photo of response.photos) {
                let response = await fetch(
                    "http://127.0.0.1:5000/img/" + photo["url"]
                );
            ...
            return (
                <>
                    <Helmet>
                        <title>
                            {" "}
                            {recipeName} by {firstName} {lastName} - MyRecipes{" "}
                        </title>
                    </Helmet>
                    <Container
